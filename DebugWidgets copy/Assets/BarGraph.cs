...
public class BarGraph : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            signedDisplay = true;
            UnityEngine.Vector3 v_local_pos = BarTransform.localPosition;
            v_local_pos.x = width / 2;
            BarTransform.localPosition = v_local_pos;
        }
    ...
    public void SetWidthPercent(float value, Color c)
    {
        ...
        if (signedDisplay)
        {
            if (Max >= 1)
            {
                UnityEngine.Vector3 v_local_scale = BarTransform.localScale;
                v_local_scale.x = value / 2;
                BarTransform.localScale = v_local_scale;
            }
            else
            {
                UnityEngine.Vector3 v_local_scale = BarTransform.localScale;
                v_local_scale.x = value * (1 / Max) / 2;
                BarTransform.localScale = v_local_scale;
            }
        }
        else
